...
Two scoping notes: only feedback from roughly the last ~16 months (the four most recent completed cycles) is surfaced — older captured reviews are not walked back to. And when `report --month` auto-backfills a missing weekly as a side effect, that backfilled `Reports/<week>.md` is written *without* the block (the monthly itself already carries it, and backfilling shouldn't fan out into one model call per week); a weekly generated directly via `report --week` does include it.
