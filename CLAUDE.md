...
- **Reading state** is `await readState()`. State has `{ status, dateStr, sessionNumber, task, ticketId, branch, cwd, repo, prNumber, claudeSessionId, startedAt, closedAt }`. There's no pid liveness — sessions are framed entirely by hooks.
