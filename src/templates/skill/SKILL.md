...
1. **A closing summary is a tell.** If you're about to write "Done.", "X landed", "all tests pass", "PR opened", "force-push complete", "fixup squashed", "build green", "implementation complete", "I have finished", "I'm done", "wrapping up", "ready for review", "the four planned changes are in place", or any other multi-bullet wrap-up that lists files/PRs/tests — that line is itself the signal. Drop the `self-wiki note` *before* the closing summary, naming the artifact (PR number, commit hash, test count). Never let the user have to invoke `/wiki` after a completion you already announced.
